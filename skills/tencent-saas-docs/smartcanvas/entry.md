...
| 字段 | 类型 | 说明 |
|------|------|------|
| `top_level_pages` | array | 顶层页面列表（`SmartCanvasElementInfo` 数组） |
| `top_level_pages[].id` | string | 页面元素唯一标识符（即 page_id） |
| `top_level_pages[].type` | string | 元素类型，页面为 `Page` |
| `top_level_pages[].element` | string | 元素内容，JSON 格式字符串（包含标题等信息） |
| `top_level_pages[].parent_id` | string | 父元素ID |
| `top_level_pages[].children` | []string | 子元素ID列表 |
| `top_level_pages[].created_by` | string | 创建者用户ID |
| `top_level_pages[].created_at` | uint64 | 创建时间戳（毫秒） |
| `top_level_pages[].updated_by` | string | 最后更新者用户ID |
| `top_level_pages[].updated_at` | uint64 | 最后更新时间戳（毫秒） |
| `error` | string | 错误信息 |
  ...
  "top_level_pages": [
    {
      ...
      "element": "{\"title\": \"首页\"}",
      "parent_id": "",
      "children": ["block_001", "block_002"]
    }
  ]
}
```

> ⚠️ **注意**：返回的 `id` 字段即为页面的 page_id，可作为 `smartcanvas.read` 的 `page_id` 参数使用。
...
| Action | 数字值 | 说明 | id 参数 | content 参数 |
|--------|--------|------|---------|----------|
| `INSERT_BEFORE` | `1` | 在指定 Block 前插入内容 | 锚点 Block ID（必填） | MDX 格式内容（必填） |
| `INSERT_AFTER` | `2` | 在指定 Block 后插入内容 | 锚点 Block ID（为空则追加到文档末尾） | MDX 格式内容（必填） |
| `DELETE` | `3` | 删除指定 Block | 要删除的 Block ID（必填，⚠️ 必须先通过 find/read 获取） | 不需要 |
| `UPDATE` | `4` | 修改指定 Block 的内容 | 要修改的 Block ID（必填，⚠️ 必须先通过 find/read 获取） | 新的 MDX 格式内容（必填） |

> ⚠️ **强制约束**：`UPDATE`（4）和 `DELETE`（3）操作的 `id` 参数**必须**来源于 `smartcanvas.find` 或 `smartcanvas.read` 的返回结果，**禁止**在未获取文档数据的情况下直接传入 id 执行 UPDATE 或 DELETE 操作。
...
|------|------|------|------|
| `file_id` | string | ✅ | 智能文档的唯一标识符 |
| `action` | integer | ✅ | 操作类型（`SmartCanvasEditAction` 枚举值）：1-INSERT_BEFORE，2-INSERT_AFTER，3-DELETE，4-UPDATE |
