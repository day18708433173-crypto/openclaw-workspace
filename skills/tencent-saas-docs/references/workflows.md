...
2. 调用 create_space_node 在目标位置创建文档节点（doc_type 按需指定，如 smartcanvas / doc / sheet / slide 等）
...
3. 调用 manage.manage.get_content 获取文档内容
```
...
#### 2. manage.scrape_progress

**功能说明**：查询网页剪藏任务进度并自动创建智能文档，与 `manage.scrape_url` 配合使用。
...
**参数说明**
- `task_id` (string, 必填): `manage.scrape_url` 返回的异步任务ID
...
```json
{
  ...
  "title": "示例网页标题",
  "file_id": "doc_1234567890",
  "file_url": "https://saas.docs.qq.com/doc/DV2h5cWJ0R1lQb0lH",
  "error": "",
  "trace_id": "trace_1234567890"
}
```
...
```
1. 调用 manage.scrape_url 传入网页URL，获取 task_id
2. 立即调用 manage.scrape_progress 传入 task_id 查询进度（每隔2秒轮询一次）
