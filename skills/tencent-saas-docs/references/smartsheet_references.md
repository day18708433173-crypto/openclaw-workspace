...
| `properties.sheet_id` | string | | 工作表唯一标识符 |
| `properties.title` | string | | 工作表标题 |
| `properties.index` | uint32 | | 工作表索引 |
...
| `property_image` | object | | 图片类型属性（无需额外配置）               |
...
| `property_phone_number` | object | | 电话号码类型属性（无需额外配置）             |
| `property_email` | object | | 邮件类型属性（无需额外配置）               |
| `property_created_user` | object | | 创建人类型属性（无需额外配置，系统自动填充）       |
| `property_modified_user` | object | | 最后编辑人类型属性（无需额外配置，系统自动填充）     |
| `property_created_time` | object | | 创建时间类型属性（系统自动填充,需指定format格式） |
| `property_modified_time` | object | | 最后编辑时间类型属性（系统自动填充,需指定format格式）          |
| `property_reference` | object | | 关联类型属性                       |
| `property_auto_number` | object | | 自动编号类型属性                     |
| `property_currency` | object | | 货币类型属性                       |
| `property_percentage` | object | | 百分比类型属性                      |
...
| `createdTime` | 创建时间 | `property_created_time` | 系统自动填充，需指定format格式        |
| `modifiedTime` | 最后编辑时间 | `property_modified_time` | 系统自动填充，需指定format格式          |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `sub_id` | string | 关联的工作表ID |
| `field_id` | string | 关联的字段ID |
| `is_multiple` | bool | 是否允许多选关联 |
| `view_id` | string | 关联的视图ID |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `type` | uint32 | 编号类型：1-自增数字类型，2-自定义类型 |
| `rules` | []NumberRule | 自定义规则列表，type 为 2 时生效 |

**NumberRule 结构**：

| 字段 | 类型 | 说明 |
|------|------|------|
| `type` | uint32 | 规则类型：1-自增ID，2-固定字符，3-创建时间 |
| `value` | string | 规则值 |

### CurrencyFieldProperty（货币字段属性）
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `currency_type` | uint32 | 货币类型：1-CNY人民币，2-USD美元，3-EUR欧元，4-GBP英镑，5-JPY日元，6-KRW韩元，7-HKD港元，8-MOP澳门元，9-TWD台币，10-AED迪拉姆，11-AUD澳元，12-BRL巴西雷亚尔，13-CAD加元，14-CHF瑞士法郎，15-IDR印尼盾，16-INR印度卢比，17-MXN墨西哥比索，18-MYR马来西亚林吉特，19-PHP菲律宾比索，20-PLN波兰兹罗提，21-RUB俄罗斯卢布，22-SGD新加坡元，23-THB泰铢，24-TRY土耳其里拉，25-VND越南盾 |
| `decimal_places` | uint32 | 小数位数，范围 0-4 |
| `use_separate` | bool | 是否使用千分位分隔符 |

### PercentageFieldProperty（百分比字段属性）
...
- **图片字段写入**：向图片类型字段（field_type=image）写入数据时，需先调用 `manage.upload_image` 工具上传图片获取 `image_id`，再以 `[{"image_id": "xxx"}]` 格式填入字段值
