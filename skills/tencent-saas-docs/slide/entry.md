...
> **创建空白 PPT 的入口**：如果用户希望从一份不存在的 PPT 开始，先用 `manage.create_file`（file_type=`slide`）创建空白 PPT，拿到 `file_id` / `file_url` 后再走本工作流。创建出来的 `file_url` 在本次任务完成时必须按第 6.1 节展示给用户。
...
- 链接来源优先级：
  ...
  2. 用户只给了 `file_id`，则拼成 `https://saas.docs.qq.com/slide/<file_id>`。
  3. 通过 `manage.create_file`（file_type=`slide`）创建出来的新 PPT，直接使用接口返回的 `url`。
- 链接展示格式：用 Markdown 链接，例如 `[在腾讯文档中打开](https://saas.docs.qq.com/slide/<file_id>)`，不要只贴裸 URL，也不要藏在折叠区或代码块里。
