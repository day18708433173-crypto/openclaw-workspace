...
#
# 功能：
#   1. 检查 mcporter 是否已配置 tencent-saas-docs（含 Authorization 可用）
#   2. 未配置或 Token 失效时，展示授权链接并等待用户主动确认已完成授权
#   3. 用户确认后主动查询一次 Token 并写入 mcporter 配置
#   4. 对过期、错误等场景给出友好提示
#   5. 同时配置 slide-mcp / doc-mcp / sheet-mcp 服务（slideengine / docengine / sheetengine 独立 MCP 入口，共享同一 Token）
#
# 用法（供 AI Agent 调用）：
#   第一步：检查状态（立即返回，不阻塞）
...
# ── 全局配置 ──────────────────────────────────────────────────────────────────
_TDOC_API_BASE="${TDOC_API_BASE_URL:-https://saas.docs.qq.com}"
_TDOC_AUTH_BASE="${TDOC_AUTH_BASE_URL:-https://saas.docs.qq.com/scenario/open-claw.html}"
_TDOC_MCP_URL="https://saas.docs.qq.com/api/v6/open/agent/mcp"
_TDOC_SERVICE_NAME="tencent-saas-docs"
_TDOC_SLIDE_MCP_URL="https://saas.docs.qq.com/api/v6/slide/mcp"
_TDOC_SLIDE_SERVICE_NAME="slide-mcp"
_TDOC_DOC_MCP_URL="https://saas.docs.qq.com/api/v6/doc/mcp"
_TDOC_DOC_SERVICE_NAME="doc-mcp"
_TDOC_SHEET_MCP_URL="https://saas.docs.qq.com/api/v6/sheet/mcp"
_TDOC_SHEET_SERVICE_NAME="sheet-mcp"
...
# 同时注册 tencent-saas-docs / slide-mcp / doc-mcp / sheet-mcp 四个服务，使用相同的 Token
_tdoc_save_token() {
    # 添加 MCP 配置
    echo "🔧 配置 mcporter..."
    ...
    # 使用传入的 token 写入 mcporter 配置（tencent-saas-docs）
    ...
    # 使用相同的 token 写入 mcporter 配置（doc-mcp，docengine 独立 MCP 服务）
    mcporter config add "$_TDOC_DOC_SERVICE_NAME" "$_TDOC_DOC_MCP_URL" \
        --header "Authorization=$token" \
        --transport http \
        --scope home

    # 使用相同的 token 写入 mcporter 配置（sheet-mcp，sheetengine 独立 MCP 服务）
    mcporter config add "$_TDOC_SHEET_SERVICE_NAME" "$_TDOC_SHEET_MCP_URL" \
        --header "Authorization=$token" \
        --transport http \
        --scope home
    ...
    if mcporter list 2>&1 | grep -q "$_TDOC_SERVICE_NAME"; then
        echo "✅ tencent-saas-docs 配置验证成功！"
        echo ""
        mcporter list | grep -A 1 "$_TDOC_SERVICE_NAME" || true
    else
        echo "⚠️  tencent-saas-docs 配置验证失败，请检查网络或 Token 是否有效"
    fi
    ...
    if mcporter list 2>&1 | grep -q "$_TDOC_DOC_SERVICE_NAME"; then
        echo "✅ doc-mcp 配置验证成功！"
        echo ""
        mcporter list | grep -A 1 "$_TDOC_DOC_SERVICE_NAME" || true
    else
        echo "⚠️  doc-mcp 配置验证失败，请检查网络或 Token 是否有效"
    fi
    if mcporter list 2>&1 | grep -q "$_TDOC_SHEET_SERVICE_NAME"; then
        echo "✅ sheet-mcp 配置验证成功！"
        echo ""
        mcporter list | grep -A 1 "$_TDOC_SHEET_SERVICE_NAME" || true
    else
        echo "⚠️  sheet-mcp 配置验证失败，请检查网络或 Token 是否有效"
    fi
    ...
    echo "   mcporter call ${_TDOC_DOC_SERVICE_NAME}.doc.insert_markdown"
    echo "   mcporter call ${_TDOC_SHEET_SERVICE_NAME}.sheet.set_cell_value"
    echo ""
...
tdoc_fetch_token() {
    ...
    local url="${_TDOC_API_BASE}/oauth/v3/mcp/get_mcp_token_by_code?code=${code}"
...
#        "400007")
#            # VIP 权限不足
#            echo "ERROR:vip_required - 当前操作需要腾讯文档 VIP 权限，请升级 VIP：https://saas.docs.qq.com/vip?immediate_buy=1?part_aid=persnlspace_mcp"
#            return 1
#            ;;
        *)
            local expired
...
_tdoc_interactive_setup() {
    ...
    # 检查服务状态
    echo "🔍 检查 tencent-saas-docs 服务配置..."
    _tdoc_check_service
    local status=$?

    case $status in
        0)
            echo "✅ tencent-saas-docs 服务已配置且运行正常！"
            echo ""
            echo "🎉 无需重新配置，您可以直接使用腾讯文档功能。"
            echo ""
            echo "📖 使用示例："
            echo "   mcporter call tencent-saas-docs manage.recent_online_file --args '{\"num\":10}'"
            return 0
            ;;
        1|2)
    ...
    case "$result" in
        ...
        ERROR:expired*)
            echo ""
            echo "❌ Token 已过期，请访问 https://saas.docs.qq.com/scenario/open-claw.html 重新获取 Token，然后重新授权"
            exit 1
            ;;
        ERROR:token_invalid*)
            echo ""
