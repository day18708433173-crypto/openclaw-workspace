...
| -------- | ----------------------------------------------------------------------------- |
| 所属服务 | `doc-mcp`                                                                     |
| 服务地址 | `https://saas.docs.qq.com/api/v6/doc/mcp`                                     |
...
> ⚠️ **所有 `doc.*` 工具均支持 `file_id` / `file_url` 二选一**。若用户提供的是文档链接（形如 `https://saas.docs.qq.com/doc/<file_id>`），可直接传 `file_url`，或先从链接末尾解析出 `file_id` 再调用。
>
...
- `file_url` (string, 可选): 腾讯文档链接（形如 `https://saas.docs.qq.com/doc/<file_id>`）
...
--tool-args '{"file_url":"https://saas.docs.qq.com/doc/XXX","idx":283,"width":580,"height":360}'
```
...
--file-url "https://saas.docs.qq.com/doc/XXX" \
    --idx 283 \
    --old-image-url "https://docimgN.docs.qq.com/image/xxx.png"
```
...
```json
{
      "file_url": "https://saas.docs.qq.com/doc/xxxxxxxx",
      "idx": 12,
...
- SaaS 端: `https://saas.docs.qq.com/api/v6/doc/mcp`
...
python3 create_with_markdown.py \
    --mcp-url "https://saas.docs.qq.com/api/v6/doc/mcp" \
...
def main():
    ...
    parser.add_argument("--mcp-url", default="",
                        help="MCP endpoint URL (必填，如 https://saas.docs.qq.com/api/v6/doc/mcp)")
...
```json
{
  "file_id": "new_doc_file_id",
      "file_url": "https://saas.docs.qq.com/doc/new_doc_file_id",
  "title": "我的新文档",
  "last_index": 100
}
```
...
- **所有 `doc.*` 工具均支持 `file_id` / `file_url` 二选一**；若用户提供的是文档链接（形如 `https://saas.docs.qq.com/doc/<file_id>`），可直接传 `file_url`，也可先解析出 `file_id` 再传入
